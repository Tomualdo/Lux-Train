...
def agent(observation, configuration):
    ...
    for unit in player.units:
        actions.append(annotate.text(unit.pos.x, unit.pos.y, f"left:{unit.get_cargo_space_left()}",42)) 
        if unit.is_worker() and unit.can_act():
            
            if unit.get_cargo_space_left() > 0:

                closest_resource_tile = get_closest_resource_tile(unit, resource_tiles, player)
                ...
                if get_city_lowes_fuel(player) > 300 :
                    closest_empty_tile = get_closes_empty_tile(unit, empty_tiles, player)
                    actions.append(annotate.circle(closest_empty_tile.pos.x, closest_empty_tile.pos.y))
